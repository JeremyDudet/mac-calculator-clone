...
const App = () => {
  ...
  const [previousFirstVariable] = useState('');
  const [previousSelectedOperation, setPreviousSelectedOperation] =
    useState('');
    ...
    if (selectedOperation === '') {
      if (firstVariable === '' || firstVariable === '0') {
        ...
        if (event.currentTarget.textContent === '.') {
          return firstVariable?.includes('.')
            ? firstVariable
            : setFirstVariable(
                firstVariable.concat(event.currentTarget.textContent)
              );
        }
      ...
      if (secondVariable === '' || secondVariable === '0') {
        ...
        if (event.currentTarget.textContent === '.') {
          return secondVariable.includes('.')
            ? null
            : setSecondVariable(
                secondVariable.concat(event.currentTarget.textContent)
              );
        }
        return setSecondVariable(
          secondVariable.concat(event.currentTarget.textContent)
        );
      }
    }
  ...
  function handleOperationClick(event) {
    if (firstVariable === '') {
      ...
      if (selectedOperation === '+') {
        setSecondVariable('');
        setFirstVariable(
          (parseFloat(firstVariable) + parseFloat(secondVariable)).toString()
        );
        setPreviousSelectedOperation(selectedOperation);
        setSelectedOperation(event.currentTarget.textContent);
  ...
  function handleEquals() {
    if (
      ...
      if (selectedOperation === '+') {
        ...
        setFirstVariable(
          (parseFloat(firstVariable) + parseFloat(firstVariable)).toString()
        );
        setPreviousSelectedOperation(selectedOperation);
        setSelectedOperation('');
      } else if (selectedOperation === '−') {
      ...
      firstVariable !== '' &&
      selectedOperation === '' &&
      previousSelectedOperation !== '' &&
      previousSecondVariable !== ''
    ) {
      if (previousSelectedOperation === '+') {
        setFirstVariable(
          (
            parseFloat(firstVariable) + parseFloat(previousSecondVariable)
          ).toString()
        );
  ...
  return (
    <>
      <SocialIcon>
        <SocialIconLink
          href='https://github.com/JeremyDudet/mac-calculator-clone'
          target='_blank'
          aria-label='GitHub'
        >
          <FaGithub size={'1.2em'} />
      ...
      <Draggable>
        <Calculator>
          <Display>
            <NumberContainer style={handleDisplayFontSize()}>
              {displayedNumber}
            </NumberContainer>
          </Display>
          <Buttons>
            <Button onClick={handleClearClick} className='modifier AC'>
              {handleClearButtonDisplay()}
            </Button>
            <Button
              onClick={handleChangeSignClick}
              className='modifier changeSign'
            >
              <Img src={img} />
            </Button>
            <Button onClick={handlePercentageClick} className='modifier %'>
              %
            </Button>
            <Button
              onClick={(event) => handleOperationClick(event)}
            ...
            <Button onClick={handleEquals} className='operation equals'>
              =
            </Button>
          </Buttons>
        </Calculator>
